...
class BackSeat():
    ...
    def run(self):
        ...
        # connect the client
            ...
            while True:
                ...
                command = self.__autonomy.decide(self.__auv_state, green, red, sensor_type='ANGLE')
                
                #store data for mission reconstruction
                self.__reconstruction.auv_command(command)
                self.__reconstruction.add_to_csv()
    ...
    def payload_command(self, command):
        ...
        if split_command[0] == 'LEFT':
            heading = str(int(float(split_command[1]))*-1)
